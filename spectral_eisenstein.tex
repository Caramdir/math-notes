...
\begin{Lem}
    There is a canonical isomorphism $\Eis P = \Eis[!]P \cong \DSerre[\Loc_G] ∘ \Eis[*] P ∘ \DSerre[\Loc_L]$ as functors $\catCoh{\Loc_L} → \catCoh{\Loc_L}$.
    Hence the functor $\DSerre ∘ \CT[*]P \DSerre$ is left adjoint to $\Eis P$ (when restricted to coherent sheaves).
\end{Lem}

\begin{proof}
    The first assertion follows from \cite[Corollary~9.5.9]{Gaitsgory:preprint:IndcoherentSheaves}:
    \[
        \Eis[*]P ∘ \DSerre =
        p_*^\IndCoh ∘ q^! ∘ \DSerre \cong
        \DSerre ∘ p_*^\IndCoh ∘ q^*.
    \]
    The second assertion follows from the fact that $\CT[*]P$ is right adjoint to $\Eis[*]P$:
    \begin{multline*}
        (\sheaf G,\, \DSerre \Eis[*]P \DSerre \sheaf F) =
        (\Eis[*]P \DSerre \sheaf F,\, \DSerre \sheaf G) =
        (\DSerre \sheaf F,\, \CT[*]P \DSerre \sheaf G) = \\
        (\DSerre \CT[*]P \DSerre \sheaf G,\, \sheaf F).
        \qedhere
    \end{multline*}
\end{proof}

Note that since $q$ is not proper we do not have $\DSerre q_*^\IndCoh = q_*^\IndCoh \DSerre$.
...
\begin{Claim}
    The functor $\CT P[G]$ is conservative.
\end{Claim}
...
\begin{proof}
    Follows from the Bar--Beck--Lurie theorem \cite[Theorem~6.2.0.6]{Lurie:2012-draft:HigherAlgebra}.
\end{proof}
